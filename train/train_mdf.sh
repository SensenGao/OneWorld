...
: "${ONEWORLD_DIT_CHECKPOINT:?Set ONEWORLD_DIT_CHECKPOINT}"
...
torchrun "${launcher_args[@]}" train/train_dit.py \
  ...
  --resume "$ONEWORLD_DIT_CHECKPOINT" \
  --steps "${ONEWORLD_MDF_TOTAL_STEPS:-120000}" \
  --out "${ONEWORLD_MDF_OUTPUT:-outputs/mdf}" \
  --mdf \
  ${EXTRA_ARGS:-}
